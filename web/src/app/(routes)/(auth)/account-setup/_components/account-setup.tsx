...
import { ChevronRight, Loader2, Terminal, User } from "lucide-react";
...
export default function AccountSetup() {
  ...
  const { mutate: usernameMutation, isPending } = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      const errorMessage =
        (error as { response?: { data?: { message?: string } } })?.response
          ?.data?.message || "Failed to set up account";
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="pt-4">
                <Button
                  type="submit"
                  disabled={isPending}
                  className="w-full rounded-sm bg-gradient-to-r from-cyan-600 to-fuchsia-600 hover:from-cyan-500 hover:to-fuchsia-500 text-white shadow-[0_0_10px_rgba(0,255,255,0.3)] disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
                >
                  {isPending ? (
                    <>
