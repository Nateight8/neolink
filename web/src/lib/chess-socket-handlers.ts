...
export function setupChessSocketHandlers(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('joinChessRoom', (
      { roomId, userId, username, isPlayer }: 
      { roomId: string; userId: string; username: string; isPlayer: boolean }, 
    ...
    // Handle chess moves
    socket.on('chessMove', (
      { roomId, from, to, san, fen, userId, username }: 
      { roomId: string; from: string; to: string; san: string; fen: string; userId: string; username: string }
    ) => {
      const room = rooms.get(roomId);
      if (!room) return;
    ...
    socket.on('updateUserRole', (
      { roomId, userId, isPlayer }: 
      { roomId: string; userId: string; isPlayer: boolean }
    ) => {
      const room = rooms.get(roomId);
      if (!room) return;
